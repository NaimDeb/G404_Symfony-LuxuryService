...
class ClientFixtures extends Fixture
{
    ...
    public function load(ObjectManager $manager): void
    {
        foreach (self::CLIENTS_DATA as $key => $clientData) {
            $client = new Client();
            $client->setCompanyName($clientData['companyName'] ?? null);
            $client->setActivityType($clientData['typeOfActivity'] ?? null);
            $client->setJob($clientData['contactPosition'] ?? null);
            $client->setPhoneNumber($clientData['contactNumber'] ?? null);
            $client->setNotes($clientData['notes'] ?? null);
